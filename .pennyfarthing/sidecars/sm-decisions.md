...
- **165-1 carryover is pre-seeded into the sibling contexts (2026-07-08, Keith asked):** `context-story-165-{2,3,4,5}.md` were created early (before their setup) with a `## Carryover from 165-1` section, and the durable copy lives in `context-epic-165.md` §Carryover. `pf context create story` has NO skip-if-exists guard — when setting up 165-2..165-5, do NOT let sm-setup's `pf context create` silently clobber these (re-add the carryover from the epic context if it does, or skip re-creating a story context that already exists). Key items: plan-doc has bugs in its embedded code (hand-verify before transcribing), `ReachResult.cost` is a SUPERSET of `.reachable`, C1's production wiring test belongs to **165-3**, and `adjudicate_reach(mode)` should be tightened to a `Literal` when 165-3 consumes it.
