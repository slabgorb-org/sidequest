...
- **No flickering data-gated tabs.** Gate a tab's *existence* on a STABLE world-capability signal (does this world deliver this data class at all — e.g. region-mode/room_graph for Location), never on a transient "have we received a message yet" signal; show empty/loading states between updates. (2026-05-21: Location tab flickered because it was gated on `currentLocation`, which re-baselines to null on every reconnect.) The `dataGated` pattern hides never-applicable tabs, not blinks applicable ones.
- **Player real names render ONLY in multiplayer.** In single-player omit entirely — no em-dash, no parenthetical, no "(you)" (in SP the player is Keith; labeling his own character is noise). Gate on `seated_player_count > 1`; test both MP-shows and SP-omits. Applies to CharacterPanel/Sheet/Widget; NPCs never show a player name.
- **Don't normalize world tone-axis vocabularies.** Each world authors its OWN `axis_snapshot` vocabulary, which is its tonal fingerprint (glenross: cosy/puzzle/gossip/gothic) — Keith called normalization "flavor loss" (2026-05-19). If a UI renderer can't read a world's axis names, fix the renderer (generalize to "low {axis}"/"high {axis}"), don't change the world or add a canonical-5. The hardcoded list in `sidequest-ui/src/screens/lobby/toneAxes.ts` is an implementation artifact, not source of truth.
- **Design-tool affordances aren't features.** claude.ai/design bundles often include the tool's own authoring affordances (tweak panels, treatment/theme switchers, density sliders) used to compare variants — NOT product features. Classify each surface as Styling (ships), Behavior (ships), or Authoring affordance (default: drop, ask before including). (Keith pushed back hard 2026-05-24 on a reference-pages "tweaks panel.")
