...
- **Design decision MADE (2026-06-07, approved):** hybrid — project engaged pool members immediately, promote to roster at milestone (tier escalation OR narrator valence beat); seen-gate = scene-present + ≥1 deduped interaction. See `docs/superpowers/specs/2026-06-07-pool-relationship-projection-promotion-design.md` — it supersedes the open question below, which is preserved for rationale.
- Original framing: pool→roster **promotion** on sustained engagement vs. direct **pool projection**. The Architect leans promotion-shaped because the machinery exists (Reuse-First): the ADR-128 development ladder in `sidequest/game/npc_development.py` already computes `acquaintance` at 3 interactions and `established` at 8, and #742 made tier escalation the only thing that writes milestone beats. A pool member crossing `acquaintance` is a natural promotion trigger. But evaluate both — direct projection avoids mutating the roster and may be the smaller change. Document the decision in the session file (or a short ADR if it changes the entity model).
