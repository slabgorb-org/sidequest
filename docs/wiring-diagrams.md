...
19. [Procedural Megadungeon](#19-procedural-megadungeon-adr-106) — beneath_sunden surface anchor → Expansion Engine → Complication Ledger
...
flowchart TD
    ...
    V --> W["+ PARTY_STATUS<br/>+ CONFRONTATION / CONFRONTATION_OUTCOME<br/>+ CHAPTER_MARKER<br/>+ TACTICAL_STATE (where Epic 29 used)<br/>(inventory in state_delta; live map view retired)"]
...
**Live vs draft:** `SIDEQUEST_GENRE_PACKS` always points at `sidequest-content/genre_packs/`, now the single tree — the `genre_workshopping/` staging tree was retired 2026-06-03 (and `low_fantasy` deleted with it). All 11 production packs load (`caverns_and_claudes`, `elemental_harmony`, `heavy_metal`, `mutant_wasteland`, `neon_dystopia`, `pulp_noir`, `road_warrior`, `space_opera`, `spaghetti_western`, `tea_and_murder`, `wry_whimsy`); `heavy_metal`, `neon_dystopia`, `pulp_noir`, and `road_warrior` load but have not cleared the asset gate. In-progress worlds stage in place via `draft: true` (currently just `tea_and_murder/blackthorn_moor`). See `docs/genre-pack-status.md`.

**caverns_and_claudes is now a procedural-megadungeon pack.** Its sole live world `beneath_sunden` (Plan 8, replaces the deprecated `caverns_sunden` hamlet) authors only the surface anchor (Ropefoot waiting-camp + Dropmouth shaft head). The deep is generated by the ADR-106 expansion engine at runtime — see §19 below. See `docs/genre-pack-status.md`.
...
**Phase-3 forward seam:** `mechanical_strip` / `fold_mechanical_strip`
ship unwired-but-annotated. Macro strip is client-side `mechStateCache`;
the server-side fold is queued for Phase 3 when a consumer exists.

**Hot-path cost guard:** Phase 2 added a wiring test asserting one turn
writes N=1 census rows per seated PC. Regression here means the census
is being called multiple times per turn — kills save-write throughput.

**Known hazard, untouched-but-flagged:** `_maybe_persist_encounter_row`
still rides outside the NARRATION tx. Phase 3 work.

---

## 18. Aside Channel (ADR-107)

A non-turn-consuming OOC question lane. The asker still owes their normal
action for the turn to resolve. Does **not** fire the ADR-036 submit-and-wait
barrier. Routes through a separate Haiku call site (`AsideResolver`),
broadcasts to the whole room (table-visible). Accepted 2026-05-18.

```mermaid
flowchart TD
    A["Client: PLAYER_ACTION<br/>{action, aside: true}"] --> B["protocol/messages.py<br/>pydantic validates payload"]
    B --> C["server/session_handler.py<br/>dispatch_player_action()"]
    C --> D{"aside == true?"}

    D -->|"yes"| E["agents/aside_resolver.py<br/>AsideResolver.answer()"]
    D -->|"no"| F["normal turn path<br/>(see §1 Core Turn Loop)"]

    E --> G["llm_factory.py::_AsideLlm<br/>(single-shot Haiku 4.5,<br/>separate AsyncAnthropic)"]
    G --> H["Anthropic API:<br/>claude-haiku-4-5-20251001<br/>(no tool-use loop, no caching)"]

    H --> I["AnswerResult<br/>{question, answer, grounded_on[]}"]
    I --> J["server broadcast:<br/>ASIDE_ANSWER<br/>{asker_id, question, answer,<br/>grounded_on, round}"]
    J --> K["All clients in room<br/>(table-visible; OOC pane)"]

    A2["Client: ACTION_REVEAL<br/>{aside: true, status: composing/submitted}"] --> L["session_room.py<br/>fan-out as OOC table-talk<br/>(NOT in-fiction action;<br/>does not feed barrier)"]
    L --> K

    M["Asker still owes a turn"] -.-> F
...
**Why Haiku, not Sonnet?** SOUL.md "Cost Scales with Drama" — an aside is
the lowest-drama input in the system. Cheapest tier is correct.

**Fails loud:** `_AsideLlm` requires `ANTHROPIC_API_KEY` and raises
`LlmClientError` if missing. No silent fallback.

**`round` is for client ordering only.** It is never a turn record. The
aside does not advance the world.

---

## 19. Procedural Megadungeon (ADR-106)

`caverns_and_claudes/beneath_sunden` is a single shaft into Sünden Deep.
The world authors only the surface anchor (Ropefoot waiting-camp at the
lip + the Dropmouth shaft mouth, the documented Plan-7 expansion seam).
The dungeon below is **generated unbounded at runtime** by the
contiguous-edge-expansion engine — Python port of the maze-maker family
plus the Complication Ledger.

```mermaid
flowchart TD
    A["caverns_and_claudes/<br/>worlds/beneath_sunden/<br/>world.yaml + cartography.yaml +<br/>openings.yaml + lore.yaml"] --> B["genre/loader.py<br/>WorldLoader"]

    B --> C["surface regions:<br/>ropefoot, the_dropmouth"]
    C --> D["session start:<br/>_commit_seed (Expansion 0)<br/>persists entrance as new_nodes"]

    D --> E["player descends:<br/>room graph navigation<br/>(ADR-055)"]
    E --> F{"at frontier edge<br/>of expanded graph?"}
    F -->|"no"| G["normal room traversal"]
    F -->|"yes"| H["Expansion N:<br/>commit_expansion()"]

    H --> I["maze-maker port:<br/>Cellular family generates<br/>new_nodes"]
    ...
    N --> O["narrator turn proceeds<br/>with new geography"]
...
**Genre-truth invariant.** Sünden is grave, lethal, Moria-as-tragedy
(gravity ≥ 0.85, no winking) — enforced by `world_register.yaml`. The
name "Sünden" is the dwarfhold's name and is never explained in-fiction.

**Authored set-pieces.** Four genre-level set-piece tropes (Plan 7 §14.A,
content PR #227) anchor the procedural deep — the engine threads them
into expansions rather than improvising every encounter.

**Visual anchor.** PR #228 (content commit `67dee9c`) deprecated the
prior `caverns_sunden` hamlet world and landed `beneath_sunden`'s visual
anchor. The lobby surfaces `beneath_sunden`, not `caverns_sunden`.

**The `claude -p` curate call** is the last surviving non-narrator
narrator-shaped LLM call on the live path. Everything else moved to the
Anthropic SDK with ADR-101.
