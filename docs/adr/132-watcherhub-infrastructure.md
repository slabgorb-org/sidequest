...
(`watcher_hub.py:1-21`). The FastAPI-facing pieces (the `/ws/watcher` WebSocket
...
1. **Singleton survival across `uvicorn --reload`.** A naïve module-level
   ...
   (`watcher_hub.py:14-21`, playtest 2026-04-23).
2. **Concurrent-session cross-attribution.** A single mutable process-global
   ...
   (`watcher_hub.py:282-294`, 2026-05-29 contamination bug).
3. **In-frame vs out-of-frame persistence routing.** Whether a telemetry row
   ...
   on the same session self-deadlocks (`watcher_hub.py:266-279`).
4. **Write-amplification from ephemeral UI state.** Keystroke-level events
   ...
   with zero audience in solo play (`watcher_hub.py:325-339`).
...
`watcher_hub.py:40`), not merely a module global. At import, the module checks
`builtins` for an existing hub and reuses it; only if absent does it construct a
new one and pin it (`watcher_hub.py:258-263`). FastAPI runs one app per process,
...
`type(_existing).__name__ == "WatcherHub"` (`watcher_hub.py:259`). After
...
`bind_event_store` writes **both in lockstep** (`watcher_hub.py:301-312`): it
...
disturbing a concurrently running session (`watcher_hub.py:282-294`).
...
(its default is `None`), else the process-global (`watcher_hub.py:315-322`). A
...
`event_seq: int | None`) parameter (`watcher_hub.py:534-542, 564-568`). The
persistence routing in `_persist_turn_telemetry` is decided **solely by that
parameter** (`watcher_hub.py:398-471`):
...
- `tx is not None` → `tx.write_telemetry(event_seq=event_seq, ...)` — the row
  ...
  rolling back atomically with the event (`watcher_hub.py:444-452`).
- `tx is None` → the resolved out-of-frame sink's `record(...)` opens its own
  short session transaction with a NULL `event_seq` (`watcher_hub.py:453-463`).
...
open turn already holds) cannot be violated (`watcher_hub.py:420-423`). This
...
(`watcher_hub.py:276-279, 408-410`). That anti-pattern is gone — the caller
...
(`watcher_hub.py:361-395`).

### 4 — Ephemeral-event taxonomy (live-push only, never persisted)

`_EPHEMERAL_EVENT_TYPES: frozenset[str]` (`watcher_hub.py:335-339`, currently
...
short-circuits on these before any DB work (`watcher_hub.py:436-437`). They carry
...
NOT be persisted in ANY mode") (`watcher_hub.py:331-334`). Discrete events with
diagnostic value (`action_reveal.submitted`) are deliberately *not* in the set
and keep persisting (`watcher_hub.py:333-334`). This is load-shedding by
...
- **Never connection-state sniffing.** In-frame vs out-of-frame is decided only
  ...
  heuristic is deleted and must not return (`watcher_hub.py:276-279, 408-410`).
- **`tx` set ⇒ write through `tx`.** When a turn transaction is open the
  ...
  pooled connection while `tx` is open (`watcher_hub.py:420-423, 444-452`).
- **Ephemeral = never persisted, in any mode.** Membership in
  ...
  out, while still allowing the live GM-panel push (`watcher_hub.py:429-437`).
- **One hub per interpreter, surviving reload.** The `builtins`-pinned instance
  ...
  never orphaned (`watcher_hub.py:258-263`).
- **Telemetry never crashes a turn.** Both persistence paths are fully wrapped:
  ...
  silent fallback (`watcher_hub.py:386-395, 464-471`). The live-push path is
  ...
  evicting live subscribers (`watcher_hub.py:114-126, 144-189`).
...
- **Plain module-global singleton (no `builtins` pin).** Rejected: a fresh
  ...
  silences the dashboard (`watcher_hub.py:14-21, 248-253`). The `builtins`
...
- **Explicit session threading of the sink through every publisher.** Rejected
  ...
  contexts that have no task scope (`watcher_hub.py:282-298, 315-322`).
- **Connection-state sniffing for in-frame detection** (`conn.in_transaction` /
  ...
  (`watcher_hub.py:276-279, 408-423`).
...
- **ADR-031 (Game Watcher — Semantic Telemetry):** governs the **model** — the
  ...
  emission path for ADR-031's semantic events (`watcher_hub.py:534-582`).
- **ADR-090 (OTEL Dashboard Restoration):** governs the post-port restoration of
  ...
  drop-on-no-loop behavior implement (`watcher_hub.py:94-97, 114-126`). This ADR
...
- **ADR-103 (Native OTEL via Tool Registry):** governs the OTEL tool-registry
  ...
  `watcher_hub.py:497-531, 581-582`) mints a zero-duration OTEL span per
  ...
  and skips re-publishing them (`watcher_hub.py:42-55`). That bridge is the seam
