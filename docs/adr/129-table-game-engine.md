...
(`engine.py:51`). It applies each committed action in resolution `order`,
ignoring folded/out seats (`engine.py:67-84`), then decides whether the hand
goes to showdown — `len(active) <= 1 or decision_point >= max_decision_points - 1`
(`engine.py:86-87`) — or advances `decision_point` and returns
(`engine.py:88-98`). `deal_table` enforces **≥2 seats** before dealing
(`engine.py:41-48`); a one-seat table raises `TableNeedsOthersError`, which
"generalizes ADR-116" (`types.py:18-22`) — no solitaire table.
...
the pot directly (`engine.py:30-32, 110-118`). The dual tension dials are never
touched — the engine reads/writes only `table_state` (module docstring,
`engine.py:1-9`).

### Sealed-commit generalization (from ADR-077)

`TableCommit` (`types.py:73-87`) is one seat's sealed action for a decision
...
read/accuse target. `TableState` (`types.py:45-70`) is the *general*
free-for-all — `seats`, a `pot`, a resolution `order` — and is **ephemeral**: a
single dramatic hand, never persisted across hands (`types.py:1-8`). What lets
poker and auction share one model is `TableSeat.private_state`
(`types.py:25-34`): an opaque dict the kind-specific resolver interprets (poker
...
`private_state` keys and beats (`auction.py:1-8`).

### Cheat / read / accuse sub-game

Signature beats dispatch through `_apply_signature_beat` (`engine.py:129-175`):
...
- **`read_table` / `read_room`** — `game.read()` returns the target's *real*
  ...
  it from everyone else (`engine.py:142-164`).
- **`accuse`** — RECORDED at commit time but **ROLLED at showdown**
  (`engine.py:165-171`, `_showdown` `engine.py:187-214`). The opposed check is
  `accuser_d20 + perception >= 10 + concealment - round(cheat_trace * 8)`
  (`engine.py:36-39, 197-199`). Because it rolls against the **final** trace, a
  ...
  hand (`engine.py:208-213`).

### NPC policy

`npc_policy.py::decide_npc_commit` (`npc_policy.py:56-107`) gives NPC seats a
...
(`npc_policy.py:35-53, 110-175`), so an auction NPC can *never* emit a poker-only
`cheat` beat the auction kind doesn't implement.

### Registry

`registry.py` holds per-kind `TableGame` resolvers (`deal`, `strength`, optional
`cheat`/`read`). Kinds register at import (`poker.py:164`, `auction.py:52`); an
unknown `game_kind` raises `UnknownTableGameError` — **no silent default game**
(`registry.py:54-60`). Cheat/read are `NotImplementedError` by default so a kind
opts in (`registry.py:35-41`); auction deliberately registers neither
(`auction.py:5-8`).
...
- **Per-seat `private_state` isolation.** Each seat's hand/valuation and its
  ...
  (`types.py:25-34`, `engine.py:149-157`). Reads deposit intel into the
...
- **Deferred accusation at showdown.** Accusations are appended to
  ...
  against the **final** `cheat_trace` (`types.py:56-59`, `engine.py:165-171,
  187-214`). An accusation against a seat that has left the table lapses
  silently-by-design (the seat is gone, `engine.py:191-193`).
...
- **Fail-loud strength.** `_showdown` calls `game.strength()` on every
  ...
  (`engine.py:224-235`). A showdown with zero eligible contenders also raises
  (`engine.py:217-218`).
- **Commits validated before mutation.** A commit naming an off-table seat
  ...
  half-applied hand (`engine.py:62-65`).
- **≥2 seats.** No solitaire table (`engine.py:41-46`, generalizing ADR-116).
...
- `table.dealt` — seat_count, game_kind, stake_kind (`table.py:23-34`)
- `table.seat_seeded` — seat_id, party_name, is_pc, keys_seeded
  (`table.py:111-123`)
- `table.commit` / `table.npc_commit` — seat, beat_id/chosen_beat, amount/pot,
  strength_band (`table.py:35-60`)
- `table.cheat` — seat, strength_before, strength_after, new_trace
  (`table.py:61-73`, fired at `engine.py:134-140`)
- `table.read` — reader, target, info_returned (`table.py:74-85`,
  `engine.py:158-163`)
- `table.accuse` — accuser, target, accuser_total, **dc**, **landed**
  (`table.py:86-99`, fired in `_showdown` at `engine.py:200-207`)
- `table.fold` — seat, decision_point (`table.py:100-110`, `engine.py:107-108`)
- `table.showdown` — winner, forfeits, pot_awarded, revealed_strengths
  (`table.py:124-136`, `engine.py:241-247`)
...
(`table.py:1-10`). The `table.cheat` span carries before/after strength and the
...
- Mechanics-first players get genuine, legible crunch: a real 52-card deck dealt
  without replacement, a real packed hand ranking (`poker.py:32-97`), real trace
...
- **ADR-033 (Confrontation engine — `native.py`):** unaffected. The table engine
  does not touch the tension dials (`engine.py:1-9`) and is not a confrontation
...
- **ADR-077 (Dogfight sealed-letter):** **generalized, not superseded.** ADR-077
  ...
  the same `beat_selections` channel (`types.py:73-87`) — and lifts it to N
...
- **ADR-116 (Confrontation requires an Other):** honored and generalized — the
  ...
  "no encounter without an other" (`types.py:18-22`).
