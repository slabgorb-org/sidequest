...
|-----------|-----------|---------|------|-------|
| Trope engine — in-session tick (passive advancement, beat fire, lifecycle, activation gating) | ADR-018 Accepted | **RESTORED** (story 45-27) | P1 | `sidequest/game/trope_tick.py` (417 lines) wired at `_execute_narration_turn`. Pass A–E shipped. Lifecycle collapsed to three states (`dormant → progressing → resolved`); ADR-018 amended to reflect. |
| Trope engine — `rate_per_day` between-session advancement | ADR-018 Accepted | **RESTORED** (story 50-4) | P3 | `sidequest/game/trope_time_skip.py` Pass A2 wired; narrator emits `days_advanced` in `game_patch`; beats fire as `TimeSkipBeatEvent`; `trope.time_skip` OTEL span live; `## TIME-SKIP CONTEXT` block injected into next narrator prompt. ADR-018 all four pillars wired. |
| NPC disposition (Attitude enum + transitions) | ADR-020 Accepted | **RESTORE** (epic 50-10/11/12/13) | P1 | Currently reduced to scalar `npc.disposition: int`. Scalar-only is below tabletop-DM baseline — fails the Keith-as-player test per CLAUDE.md. Four-story restoration: central `Attitude` enum (50-10), threshold-crossing OTEL fields (50-11), narrator attitude serialization (50-12), genre-configurable thresholds (50-13). |
...
| Scenario system — `discover_clue` wiring (call site) | ADR-053 Accepted | **RESTORE** (epic 50-5) | P2 | `scenario_state.discover_clue()` exists with `SPAN_SCENARIO_ADVANCE` emission, but a grep finds zero production callers. Once `scenario_bind.py` loads the clue graph at session start, scenario state is never read or mutated for the rest of the session. Prerequisite for the other three ADR-053 items — without an input edge, the rest of the system has nothing to chew on. |
| Scenario system — ClueGraph DAG prerequisite enforcement | ADR-053 Accepted | **RESTORE** (epic 50-6, depends on 50-5) | P2 | `discover_clue()` appends to `discovered_clues` with no DAG check. The causal-discovery-order guarantee that is ADR-053's headline is unenforced. Small change inside the function once it has callers. |
| Scenario system — GossipEngine (NPC belief propagation) | ADR-053 Accepted | **RESTORE** (epic 50-7) | P2 | No `propagate_gossip`, no two-phase snapshot-then-mutate, no contradiction detection, no credibility decay. The data layer exists; the engine driving belief flow between NPCs does not. Required for multi-NPC mystery scenarios to feel alive. |
| Scenario system — AccusationEvaluator (`evaluate_accusation`) | ADR-053 Accepted | **RESTORE** (epic 50-8, depends on 50-7) | P2 | No `evaluate_accusation`, no `EvidenceSummary`, no Circumstantial/Strong/Airtight verdict computation. `SPAN_SCENARIO_ACCUSATION` is defined but never emitted. On player accusation the LLM improvises — the rejected alternative in ADR-053. |
...
| Affinity progression | ADR-021, ADR-081 | **DEFER** | — | Porter's `P6-deferred` marker confirmed at `game/character.py`. Will land with the advancement epic. No action. |
| Advancement / XP pipeline | ADR-081 Proposed | **DEFER** | — | Partial `award_turn_xp` stub; marker aligned with ADR-081's Proposed status. |
| Dogfight subsystem | ADR-077 Accepted (promoted from Proposed 2026-05-02) | **VERIFIED live** | — | Sealed-letter cross-product resolution wired via `ResolutionMode.sealed_letter_lookup` dispatch branch at `sidequest-server/sidequest/server/narration_apply.py`. Content shipping in `sidequest-content/genre_packs/space_opera/dogfight/`. SOUL gate exclusion in place. Test coverage at `tests/genre/test_dogfight_content_loading.py`. No restoration owed. |
| Edge/Composure push-currency rituals + Epic 39 wiring | ADR-078 Accepted (promoted from Proposed 2026-05-02) | **VERIFIED partial** | P2 | Edge primitive live: `EdgePool` on `CreatureCore`, `apply_edge_delta` called from `dispatch/yield_action.py` and `game/session.py, 888`; shared threshold helper at `game/thresholds.py`; `BeatDef.edge_delta` field at `genre/models/rules.py`; advancement-effect data shapes at `genre/models/advancement.py–106`. Gaps: (1) **Epic 39** — per-class edge config wiring; `world_materialization.py` placeholder explicit about it; (2) `composure_break` OTEL span (§4 of ADR) not emitted — engine-derived resolution at edge≤0 not wired; (3) push-currency content (`pact_working`/`commit_cost`) lives in `sidequest-content/genre_workshopping/heavy_metal/`, not shipped to production `genre_packs/`. Plus vestigial HP fields linger (`tension_tracker.py–350`, `history_chapter.py`) — stale-schema bugs per project memory `[HP removed per ADR-014]`. |
