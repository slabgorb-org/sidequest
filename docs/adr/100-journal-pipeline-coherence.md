...
## Amendment (2026-06-22, story 153-32) — The footnote stream is the ephemeral journal feed by design; there is no general footnote → `known_facts` route

A full-stack `/sq-playtest` forensics snapshot on a non-scenario world (`beneath_sunden`) surfaced what read like a bug: after three turns the Knowledge/Journal UI showed six entries, yet `snapshot.characters[].known_facts` was **empty** and `/timeline` carried no `KNOWLEDGE` event kind. Story 153-32 was opened to decide whether the per-turn narration `footnotes` stream **must** be persisted into server `known_facts` (so the narrator is fed what the player knows and the Journal survives a cold reload), or whether client-reactive-only is the intended design.

**Decision: client-reactive-by-design (WONTFIX). The per-turn `footnotes` stream is the ephemeral journal feed; there is deliberately no general "persist every footnote" route into `known_facts`.** This ADR is the controlling authority for the journal (ADR-136 governs the **NPC relationship** surface — disposition / OCEAN / belief-claims — not the player knowledge journal; the story's "per ADR-136" framing was a misattribution). The decision rests on what this ADR already architected:

- **Two channels, by design.** Component 2 (footnote forwarding to the UI) is "the per-turn ephemeral channel." Component 3 (`Character.known_facts`) is the durable store, written **only by deliberate paths** — the scenario clue hook (Seam A/B), the narrator's `commit_known_fact` tool, and `WorldStatePatch.discovered_facts`. The narrator prompt (`agents/narrator_prompts/output_only.md`) makes the contract explicit to the model: *"emit footnotes here AND call `commit_known_fact` when the fact should be durably known."* The footnote feed is "include generously"; the durable commit is the narrator's Diamonds-&-Coal judgment of what is worth keeping.
- **The only footnote → `known_facts` route is the structured clue match,** and it is intentionally narrow: `consume_clue_footnotes` mints a `KnownFact` only when `fn.fact_id` resolves to a `ClueNode` in a bound `scenario_state` (no-op when `scenario_state is None`). Adding a general footnote dump would be the **fourth state-mutation pathway** this ADR's *Alternatives Considered* already rejected, and would make the footnote feed an indiscriminate third writer of `known_facts` — violating the "deliberate writers only" discipline named in *Consequences → Negative*.
- **The narrator is not pre-fed `known_facts`** (confirmed: no prompt section injects it; `query_known_facts` is the on-demand read that *"replaces ad-hoc known_facts blocks dumped into the narrator prompt"*). That is the intended design, not a gap.
- **Cold reload.** `pg/snapshot.load_snapshot` rehydrates persisted `known_facts` and feeds it into `_generate_recap`, so durable facts (e.g. `commit_known_fact` / scenario clues) survive a cold load and reach the recap. The **ephemeral UI Journal feed** does not survive a cold reload — there is no footnote replay-backfill on resume — but that durability is **Seam C's** job (`JOURNAL_RESPONSE`, feeder stories 50-14..50-17), not a reason to start dumping footnotes into `known_facts`.

**Consequences of the decision:** no behavior change. The empty `known_facts` / populated-Journal pattern on a no-scenario-graph world is the system honestly reporting its two-channel design, and the GM panel can already distinguish the channels via existing watcher events: the feed emits the `state_transition` (field=`footnotes`) watcher event — plus `state.footnote_fact_id_minted` — through `_watcher_publish` in `websocket_session_handler.py`, while durable mints emit the `tool.write.commit_known_fact` dispatch span and `SPAN_SCENARIO_ADVANCE` (`"scenario.advance"`). (`state.footnotes_forwarded` is a `logger.info` label, not a watcher event — the panel reads `_watcher_publish`, not logs.) So no new `KNOWLEDGE` timeline event kind is warranted. The contract is recorded here and at the forwarding seam (`server/websocket_session_handler.py`, immediately after the `consume_clue_footnotes` call) so the next forensics reader is not surprised. The genuinely-open durability question (UI Journal rehydration on cold reload) remains owned by the Seam C feeder stories above, unchanged by this decision.
