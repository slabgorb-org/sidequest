...
## Anchor calibration tooling & per-world status (follow-on, 2026-07-09)

The manual "calibrate `node_anchors` against the real scan" step (the Task 9–11
`assets/maps/README.md` checklists) now has a visual authoring tool:
`sidequest-content/tools/map-anchor-calibrator.html` — generated by
`build_map_calibrator.py`, documented in `tools/MAP_CALIBRATOR.md`. Pick a world,
load a public-domain scan under the pins, drag each region to its true pixel,
export a paste-ready `map.yaml`. It covers **all 22 worlds** with a
`cartography.yaml`: the raster worlds seed from their real anchors, the rest from
a deterministic spring-layout of their adjacency graph so a brand-new `map.yaml`
can be bootstrapped. Snap-to-grid, alignment guides, distance readout, and live
rename / adjacency editing (the latter exports a `cartography.yaml` reference
patch — the tool never rewrites cartography). No runtime surface; a dev aid only.

Per-world main-map status:

| World | Pack | Treatment | Anchors | Provenance |
|-------|------|-----------|---------|------------|
| glenross | tea_and_murder | raster | placeholder (pending calibration) | NLS OS large-scale sheet (sourced) |
| annees_folles | pulp_noir | raster | **calibrated** (period Paris plan) | open: Ward, Lock vs Baedeker — PR #521 |
| the_circuit | road_warrior | raster | placeholder + wasteland-defacement style | US state highway map (163-4) |
| the_real_mccoy | spaghetti_western | raster | **calibrated** (Woods 1867 Pittsburgh) | real (NYPL, PD) |
| 18 others | — | none yet | auto-laid-out in tool | to source when a world gets a map |

**Caveat carried forward:** `node_anchors` are pixels within the working canvas,
specific to the exact scan they were calibrated on — swapping the scan requires
re-calibration. A **fractional (0–1) anchor mode** (resolution-independent,
survives scan swaps) is the recommended next refinement to the tool + the
`MapTreatmentConfig` loader/validator.
