...
7. **Async look-ahead materializer + frontier-crossing promotion** — wiring test + OTEL. ⚠ **IMPLEMENTED, NOT YET MERGED** (Plan 7 Tasks 1–8, `feat/beneath-sunden-plan-7-materializer`, server HEAD `843c085`; full server suite green at 6465 passed / 0 failed). As-built: five-stage async pipeline (`design → fill → curate → attach → commit`) in `sidequest/dungeon/materializer.py`; frontier-crossing seam in `frontier_hook.py` (hooks `GameSnapshot._apply_world_patch_inner`); async look-ahead worker in `lookahead_worker.py`; OTEL spans `dungeon.materialize.*`/`frontier.expand`/`frontier.region_transition`/`frontier.lookahead`. **OPEN FOLLOW-UP — live-session registration not done:** `lookahead_worker.register_lookahead_worker` is complete and wiring-tested against the real `apply_world_patch`→`notify_region_transition` producer, but the live WebSocket session lifecycle does NOT yet construct a `DungeonStore` / resolve bundle/palette/pack_tropes/campaign_seed at session-open and call `register_lookahead_worker(...)`. The ADR-106 "zero non-test runtime consumers" bottleneck is **REDUCED** (the consuming seam exists and is proven) but **NOT fully closed** (`observers=0` on live `frontier.region_transition` spans is the lie-detector signal). A session-integration story is required to close this. **OPEN GAP — mask persistence:** `commit_expansion` persists only `RegionNode.to_dict()`; fill grids and mask BLOBs have no write path and no Plan 1–6 loader (inert until a future plan adds a mask-BLOB column). See Post-Implementation Corrections in `docs/superpowers/plans/2026-05-16-beneath-sunden-plan-7-materializer.md` (items 10 and 14).
