...
>
> **What the fields carry, and the per-ruleset story.** `PartyMember.current_hp` / `max_hp` (PARTY_STATUS) are populated **unconditionally** from `character.core.hp.current` / `.max` (`server/views.py:506-511`) — there is no per-ruleset branch on this publish path, so they carry the HpPool for *every* pack regardless of which ruleset is bound. Genre packs select a pluggable ruleset module via `ruleset:` in `rules.yaml` (`game/ruleset/`, registered in `game/ruleset/registry.py`; unknown names fail loud — there is no default or fallback). **Six modules are registered (ADR-117/-142/-143/-144), but the binding reality is two SRD families:**
>
> - **`native`** — the legacy ADR-033/-078 dial/confrontation engine. **Vestigial: registered but bound by zero packs** — no `rules.yaml` sets `ruleset: native`, and it is *not* a default or fallback. Its dial beat-DC formula (`clamp(10 + 2·|base|, 10..30)`, the native module's `offer_difficulty`) is the one resolver no live ruleset uses. (The per-confrontation `win_condition="dial_threshold"` is a **separate, still-live** concept — see below — *not* a sign this module is bound.)
> - **Without Number family (HP-based, Sine Nomine SRDs, ADR-142/-143)** — `wwn` Worlds Without Number (`caverns_and_claudes`, `elemental_harmony`, `heavy_metal`), `cwn` Cities Without Number (`neon_dystopia`, `road_warrior`), `swn` Stars Without Number (`space_opera`), `awn` Ashes Without Number (`mutant_wasteland`). **7 packs.** All resolve combat via `hp_depletion`.
> - **`fate`** — Fate Core (CC-BY SRD, ADR-144). Resolves on stress + consequence tracks, **not** HP or dials (`pulp_noir`, `spaghetti_western`, `tea_and_murder`, `wry_whimsy`). **4 packs.**
>
> The ruleset governs **combat resolution**, not whether the HpPool exists — every `CreatureCore` carries an `HpPool` regardless of ruleset (so `PartyMember` HP fields publish for Fate packs too, even though Fate does not resolve on them). The confrontation surface is where survivability shows on the wire, but `win_condition` is **decoupled from the ruleset module** — it is stamped straight from the confrontation def (`encounter.win_condition`, `server/dispatch/confrontation.py`, ungated by ruleset) and is one of three values (`game/encounter.py`):
>   - `hp_depletion` — the Without-Number HP path; `ConfrontationPayload` (`protocol/messages.py`) adds `player_hp` / `opponent_hp` (`{"current", "max"}`).
>   - `table_showdown` — the ADR-129 poker / N-seat path (`road_warrior`, `tea_and_murder`, `spaghetti_western`).
>   - `dial_threshold` — the default for everything else (hp fields omitted). This is the native dial engine's resolution model by origin, but as the confrontation **default it fires for live, non-native packs** — e.g. the SWN sealed-letter dogfight in `space_opera` (ADR-077). A `dial_threshold` payload is *not* evidence that `native` is bound.
>
> **All eleven packs are bound (two-SRD end state, ADR-144):** 7 to the Without-Number family (`wwn`/`cwn`/`swn`/`awn`) and 4 to `fate` — see the module list above. There is no homebrew ruleset left to balance; the native dial engine survives only as the vestigial `native` module, bound by nothing. Personal Edge/Composure did **not** return as a survivability substrate — ADR-114 reversed ADR-078's composure deletion in favor of ablative HP, and the only composure pool that survives is the vessel/`rig_pool` (`rig_composure_current` / `rig_composure_max`).
>
