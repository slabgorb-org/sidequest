...
- **Anthropic SDK narrator (ADR-101) — default on `develop` 2026-05-15** — `agents.llm_factory.build_llm_client()` returns `AnthropicSdkClient` by default; native tool-use (ADR-102) replaces the ADR-039 fenced-JSON sidecar; native OTEL via tool registry (ADR-103) replaces the ADR-058 `playtest_otlp` scraper for narrator state; per-call model routing picks Haiku 4.5 (classification/scratch) or Sonnet 4.6 (narration) by purpose — the Opus 4.8 `NARRATION_IMPORTANT` rung is defined but intentionally unused (Sonnet is the effective top tier); prompt caching on stable system zones holds ~60% cached input after warmup. `claude -p` opt-in only on the narrator path; still load-bearing for daemon subject extraction and the ADR-106 megadungeon "curate" stage. `ANTHROPIC_API_KEY` is a hard runtime requirement (no silent fallback)
