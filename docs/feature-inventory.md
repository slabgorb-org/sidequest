...
**Last updated:** 2026-05-28 (pack-count reconciliation)
...
| Unified narrator (stateless turns, ADR-098) | `agents.narrator` + `agents.anthropic_sdk_client` (default, ADR-101) / `claude_client.send_stateless` (opt-in) | — | Each turn invokes the SDK as a bounded stateless call (no `--resume`, no `narrator_session_id`); per-turn prompt size stays bounded across 30-turn sessions; prompt caching on stable system zones holds ~60% cached input after warmup |
| Anthropic SDK narrator (ADR-101) | `agents.llm_factory.build_llm_client` (default `anthropic_sdk`) + `agents.anthropic_sdk_client` + `agents.model_routing` + `agents.anthropic_cost` | Dashboard cost pane | Per-turn `llm.request` OTEL span carries model + token counts + cache hits + USD cost; per-call routing picks Haiku (classification/scratch) or Sonnet (narration) by purpose (`agents/model_routing.py`); the Opus `NARRATION_IMPORTANT` rung is defined but **intentionally unused** — Sonnet is the effective top tier (ADR-101); `ANTHROPIC_API_KEY` is hard-required (no silent fallback) |
...
| Native tool-use protocol (ADR-102) | `agents.tooling_protocol` + `orchestrator.assemble_turn` + tool registry | Dashboard | On SDK backend, mechanical patches (items, mood, intent, SFX, visual_scene, etc.) arrive as JSON-Schema-validated tool calls — narrator structurally cannot describe a mechanical effect without invoking its tool; `assemble_turn` merges tool results with prose, tool values precede prose extraction |
| Native OTEL via tool registry (ADR-103) | `telemetry.spans.llm_request` + tool registry instrumentation | Dashboard / Jaeger | Each tool call emits a span automatically — no sidecar scraper; spans flow `server-tracer → gRPC → Jaeger`; legacy `playtest_otlp` (ADR-058) HTTP/JSON pipe still feeds old dashboard panes only |
| Aside resolver (ADR-107) | `agents.aside_resolver` + `llm_factory._AsideLlm` + `handlers.aside` | InputBar OOC toggle, ASIDE_ANSWER pane | Asking `/aside` or toggling OOC sends `PLAYER_ACTION{aside: true}` → Haiku 4.5 single-shot → `ASIDE_ANSWER` broadcast to room; does not consume a turn, does not fire the ADR-036 barrier, never becomes narration |
| Sidecar tool patches (legacy `claude -p` path) | `orchestrator.assemble_turn` | — | Opt-in only via `SIDEQUEST_LLM_BACKEND=claude` + `SIDEQUEST_NARRATOR_STREAMING=1`; JSONL fence parsed for items/mood/intent/SFX (ADR-039, superseded by ADR-102 on the SDK path) |
...
| Postgres persistence (ADR-115) | `game.pg.save_repository.PgSaveRepository` (`SaveRepository` Protocol, `game.repository`) + alembic | — | Save round-trip (`/character/save` → reconnect → state restored); one shared Postgres DB (`SIDEQUEST_DATABASE_URL`), rows keyed by `session_id`. SQLite survives only as the read-only importer (`game/importer.py`) |
...
| Lore store | `game.lore_store` + `game.pg.lore` | — | *Engineering* — persisted in Postgres (ADR-115) |
...
> **Reconciliation (2026-06-17, two-SRD reality — ADR-117/-143/-144).** The
> generated table above marks the confrontation engine "Live & Wired," and it
> is — but read it as the *substrate*, not the resolver. `StructuredEncounter`
> + `apply_beat` (Pillar 1) and `ResourcePool` (Pillar 2) are the live scaffold
> that every bound ruleset plugs into; the **native dial resolution** the engine
> originally drove is **vestigial** — no pack binds `ruleset: native`, and under
...
> **2026-06-12 update.** The 2026-05-28 snapshot below is superseded on several counts: there are now **11 packs** (the count below predates `wry_whimsy`) and **22 worlds, all live** — no world sets `draft: true` anymore (`blackthorn_moor` promoted; its asset gate is met with 8 POIs + 23 portraits on R2). New worlds since the snapshot: `heavy_metal/barsoom` (WWN ruleset, live 2026-06-05, assets complete), `mutant_wasteland/seaboard_of_saints` (18 POIs + 51 portraits + leitmotif music). The asset gate is met for **every** world — the "not yet met for heavy_metal / neon_dystopia / pulp_noir / road_warrior" claim below is stale. `caverns_and_claudes/beneath_sunden` gained the WWN port + 105-2 seam registry (entrance room "Under the Rope") on 2026-06-12. Current per-world matrix: [`docs/genre-pack-status.md`](genre-pack-status.md).
...
- **space_opera** — `coyote_star` (renamed from `coyote_reach` 2026-05-01) is the Magic + Rig MVP flagship; chapter→trope wiring engaged (content PR #209). **Now binds the SWN (Stars Without Number) ruleset** (`ruleset: swn`; e2e green 2026-05-27): the six native attributes are SWN-mapped and **combat — both personal (Firefight) and Ship Combat — resolves d20 + attack_bonus + combat_skill + attr_mod vs Armor Class with ablative HP and `hp_depletion` victory** (ship hull = HP, AC 14, flat armor soak), replacing the old LLM-interpreted "Ship Block" dials for combat. Honest caveats: SWN skill checks (2d6), saves, and 1d8+DEX initiative **are wired** and resolve through the bound ruleset module — checks/saves via the player-initiated `CHECK_THROW` dice protocol (`dispatch/check.py` → `ruleset.check_params`/`save_params`, no narrator run), initiative via `encounter_lifecycle` (`ruleset.roll_initiative`); the only residual limit is that the **narrator does not auto-trigger checks** — they're **player-pull**, not narrator-demanded; chargen has no per-class HP tables yet (characters default to 10 HP); non-combat confrontations (negotiation, pursuit) and the sealed-letter dogfight resolve via the `dial_threshold` win condition (the confrontation default) **under the bound SWN module** — not a separate `native` ruleset; only direct combat uses `hp_depletion`
- **tea_and_murder** — pack-level rename from `victoria` completed 2026-05 across all repos. Both worlds are live: `glenross` (Highland village Edwardian Scotland c. 1908) and `blackthorn_moor` (promoted out of draft by 2026-06-12; 8 POIs + 23 portraits on R2).

Pack-by-pack: [`docs/genre-pack-status.md`](genre-pack-status.md).
...
- **Anthropic SDK narrator (ADR-101) — default on `develop` 2026-05-15** — `agents.llm_factory.build_llm_client()` returns `AnthropicSdkClient` by default; native tool-use (ADR-102) replaces the ADR-039 fenced-JSON sidecar; native OTEL via tool registry (ADR-103) replaces the ADR-058 `playtest_otlp` scraper for narrator state; per-call model routing picks Haiku 4.5 (classification/scratch) or Sonnet 4.6 (narration) by purpose — the Opus 4.7 `NARRATION_IMPORTANT` rung is defined but intentionally unused (Sonnet is the effective top tier); prompt caching on stable system zones holds ~60% cached input after warmup. `claude -p` opt-in only on the narrator path; still load-bearing for daemon subject extraction and the ADR-106 megadungeon "curate" stage. `ANTHROPIC_API_KEY` is a hard runtime requirement (no silent fallback)
- **ADR-107 out-of-band aside channel (2026-05-18)** — `PLAYER_ACTION{aside: true}` routes to `AsideResolver` (Haiku 4.5, single-shot); answers broadcast as `ASIDE_ANSWER` to whole room; does not consume a turn or fire the ADR-036 barrier; `ACTION_REVEAL{aside: true}` mirrors composing/submitted text to peers as OOC table-talk
- **ADR-106 procedural Jaquaysed megadungeon (closed 2026-05-17)** — `caverns_and_claudes/beneath_sunden` runs the contiguous-edge-expansion engine (Python port of maze-maker Cellular family) plus Complication Ledger; surface anchor authored, deep is runtime; entrance persisted as Expansion 0 via `_commit_seed`; materializer seed-then-commit with rollback on `PersistError`
- **Forensics telemetry substrate P1 + P2 (merged 2026-05-18)** — `turn_telemetry` (per-turn back-pressure: model, latency, tokens, cost, tool counts) + `mechanical_census` (per-seated-PC edge / xp / inv / trope projections + per-round diff lanes) ride the NARRATION (`emit_event`) Postgres transaction so per-turn metrics never desync from the event they describe (ADR-115; both tables keyed by `session_id`); hot-path cost-guarded (N=1 census rows per turn); `mechanical_strip` / `fold_mechanical_strip` ship annotated as Phase-3 forward seams. Read via `PgForensicReader` (`game/pg/forensic.py`) under MVCC pooled reads — the old `open_save_readonly` / `SqliteStore.open()` SQLite path is gone
- **Stateless narrator (ADR-098)** — invoked fresh each turn; no `--resume`, no `narrator_session_id`, bounded per-turn prompt; warm-reboot/§8 recovery deleted
...
- **Ablative HP (survivability track)** — `HpPool` (`current`/`max`/`base_max`) is first-class on `CreatureCore` (ADR-114, reversing ADR-078's HP→Edge removal); damage flows through the strike channel, 0 HP → `hp_depletion`, `state_patch_hp` OTEL span per delta. Published unconditionally as `current_hp`/`max_hp` on `PartyMember` (`views.py`). The ADR-078 Edge/Composure machinery for personal creatures is retired (vessel rig-composure survives separately); Edge/Composure is planned to return as the Fate SRD substrate
...
> **2026-06-12 update.** The climb finished: **all 22 worlds across 11 packs are live and asset-complete** (POIs + portraits on R2 for every world; the only outstanding portrait gap is 7 errored `heavy_metal` pickers — 5 in `evropi`, 2 in `long_foundry` — that failed at render and need re-uploading; `heavy_metal/barsoom` is complete). No world carries `draft: true` — `blackthorn_moor` is promoted. The "loadable-but-rough" tier above has emptied into the asset-complete tier; what distinguishes worlds now is playtest depth, not assets. Per-world counts: [`docs/genre-pack-status.md`](genre-pack-status.md).
