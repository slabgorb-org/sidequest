...
- **space_opera** — `coyote_star` (renamed from `coyote_reach` 2026-05-01) is the Magic + Rig MVP flagship; chapter→trope wiring engaged (content PR #209). **Now binds the SWN (Stars Without Number) ruleset** (`ruleset: swn`; e2e green 2026-05-27): the six native attributes are SWN-mapped and **combat — both personal (Firefight) and Ship Combat — resolves d20 + attack_bonus + combat_skill + attr_mod vs Armor Class with ablative HP and `hp_depletion` victory** (ship hull = HP, AC 14, flat armor soak), replacing the old LLM-interpreted "Ship Block" dials for combat. Deferred/honest caveats: SWN skill checks (2d6), saves, and 1d8+DEX initiative are coded in the `swn` module but **not yet routed from the orchestrator**; chargen has no per-class HP tables yet (characters default to 10 HP); non-combat confrontations (negotiation, pursuit, dogfight) still use native dial rules
