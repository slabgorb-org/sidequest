...
| Anthropic SDK narrator (ADR-101) | `agents.llm_factory.build_llm_client` (default `anthropic_sdk`) + `agents.anthropic_sdk_client` + `agents.model_routing` + `agents.anthropic_cost` | Dashboard cost pane | Per-turn `llm.request` OTEL span carries model + token counts + cache hits + USD cost; per-call routing picks Haiku/Sonnet/Opus by purpose (ADR-101); `ANTHROPIC_API_KEY` is hard-required (no silent fallback) |
...
> **2026-06-12 update.** The 2026-05-28 snapshot below is superseded on several counts: there are now **11 packs** (the count below predates `wry_whimsy`) and **22 worlds, all live** — no world sets `draft: true` anymore (`blackthorn_moor` promoted; its asset gate is met with 8 POIs + 23 portraits on R2). New worlds since the snapshot: `heavy_metal/barsoom` (WWN ruleset, live 2026-06-05, portraits still rendering), `mutant_wasteland/seaboard_of_saints` (18 POIs + 51 portraits + leitmotif music). The asset gate is met for **every** world — the "not yet met for heavy_metal / neon_dystopia / pulp_noir / road_warrior" claim below is stale. `caverns_and_claudes/beneath_sunden` gained the WWN port + 105-2 seam registry (entrance room "Under the Rope") on 2026-06-12. Current per-world matrix: [`docs/genre-pack-status.md`](genre-pack-status.md).
...
- **Forensics telemetry substrate P1 + P2 (merged 2026-05-18)** — `turn_telemetry` (per-turn back-pressure: model, latency, tokens, cost, tool counts) + `mechanical_census` (per-seated-PC edge / xp / inv / trope projections + per-round diff lanes) ride the NARRATION SQLite transaction so per-turn metrics never desync from the event they describe; hot-path cost-guarded (N=1 census rows per turn); `mechanical_strip` / `fold_mechanical_strip` ship annotated as Phase-3 forward seams. Read via `open_save_readonly` — never `SqliteStore.open()`
...
> **2026-06-12 update.** The climb finished: **all 22 worlds across 11 packs are live and asset-complete** (POIs + portraits on R2 for every world; `heavy_metal/barsoom` portraits still rendering). No world carries `draft: true` — `blackthorn_moor` is promoted. The "loadable-but-rough" tier above has emptied into the asset-complete tier; what distinguishes worlds now is playtest depth, not assets. Per-world counts: [`docs/genre-pack-status.md`](genre-pack-status.md).
