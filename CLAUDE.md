...
- **Genre packs** live in `sidequest-content/genre_packs/` (single source of truth), loaded by the server from `SIDEQUEST_GENRE_PACKS`. A pack can bind one of four live pluggable ruleset modules via `ruleset:` in its `rules.yaml` (`native` default = ADR-033 dial engine; `swn` = Stars Without Number → `space_opera`; `wwn` = Worlds Without Number → `elemental_harmony`; `cwn` = Cities Without Number → `neon_dystopia`; unknown values fail loud at load)
